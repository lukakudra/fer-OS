...
int prioritet[DRETVI]={0}, id[DRETVI]={0}, t_d[DRETVI]={0}, v_t[DRETVI]={0};
...
void ispis_stanja ( int ispisi_zaglavlje ) {
	...
	if ( ispisi_zaglavlje ) {
		...
		for ( i = 0; i < MAX_DRETVI; i++ )
			if ( P[i].id != -1 && P[i].prio!=-1 && P[i].p>0)
				printf ( "  %d/%d/%d ",
...
int main (void) {
	...
	while(1) {
		...
		for(i=0; i<DRETVI; i++) {
			if(t==t_d[i]) {
				printf("%3d -- nova dretva id=%d, prio=%d, p=%d\n", t, id[i], prioritet[i], v_t[i]);
				P[i].id=id[i];
		...
		//smanjujem vrijeme trajanja najprioritetnije dretve
		P[0].p--;
		if(P[0].p==0) {
